...
void NAMFScheduler::process() {
    /* po wszystkich elementach i wywyołaj process jeśli nextRun*/
    for (byte i = 0; i < SCHEDULER_SIZE; i++) {
        if (_tasks[i].nextRun && _tasks[i].nextRun < millis()){
            unsigned long nextRun = _tasks[i].process();
...
void NAMFScheduler::init() {
    /* po wszystkich elementach i wywyołaj process jeśli nextRun*/
    for (byte i = 0; i < SCHEDULER_SIZE; i++) {
        unsigned long nextRun = _tasks[i].init();
        if (nextRun) {
            _tasks[i].nextRun = millis() + nextRun;
        } else {
            _tasks[i].nextRun = 0;
...
int NAMFScheduler::registerSensor(byte slot, loopTimerFunc initF, loopTimerFunc processF, loopJSONFunc resultF) {
    {
        if (loopSize+1 >= SCHEDULER_SIZE)
            return -1;
        _tasks[loopSize].nextRun = 0;
        _tasks[loopSize].init = initF;
        _tasks[loopSize].process = processF;
        _tasks[loopSize].results = resultF;
...
unsigned long nullF(void) { return 0; }

String nullJSONF(void) {
    String s;
    s = String("");
    return s;
}
