...
function setSSID(ssid){document.getElementById('wlanssid').value=ssid.innerText||ssid.textContent;document.getElementById('wlanpwd').value='';document.getElementById('wlanpwd').focus();}\
function load_wifi_list(r){console.log('WiFI');var x=new XMLHttpRequest();var inf=document.getElementById('inf');inf.innerHTML='Refreshing...';var u='/wifi';if(r==1){u+='?r=1'}var done=false;var timeoutId=setTimeout(function(){if(done){return;}done=true;x.abort();inf.innerHTML='';alert('WiFi list refresh timed out. Please try again.');},10000);x.open('GET',u);x.onload=function(){if(done){return;}done=true;clearTimeout(timeoutId);if(x.status===200){document.getElementById('wifilist').innerHTML=x.responseText;}else{alert('WiFi list refresh failed (HTTP '+x.status+').');}inf.innerHTML='';};x.onerror=function(){if(done){return;}done=true;clearTimeout(timeoutId);inf.innerHTML='';alert('WiFi list refresh failed. Network error.');};x.send();}\
</script>";
